    ...
    def handleTriggerQuery(self, query):
        ...
        if self.window_list:
            for window in self.window_list:             
                ...
                print(f"search_term : {search_term}, window.window_class : {window.window_class}")
                # caption 또는 desktops 값과 검색어가 일치하는지 확인
                if search_term in (f"[{window.workspace.id}] {window.title}").lower() or search_term in window.window_class.lower() :
